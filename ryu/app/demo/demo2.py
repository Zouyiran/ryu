...
class Network_Aware(app_manager.RyuApp):
    ...
    # get the adjacency matrix for switches
    ...
    def create_port_map(self, switch_list):
        for sw in switch_list:
            dpid = sw.dp.id  # the datapath's id
            self.switch_port_table.setdefault(dpid, set())  # the switch's port: dpid -> set()
            self.interior_ports.setdefault(dpid, set())  #
            self.access_ports.setdefault(dpid, set())

            for p in sw.ports:  # one switch has several ports
                self.switch_port_table[dpid].add(p.port_no)  # dpid -> set.add(port)
