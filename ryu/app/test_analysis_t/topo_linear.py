    ...
    def __init__(self,switch_num):
        ...
        for count in range(self.switch_num): #
            if count == self.switch_num - 1:
                ...
                self.addLink(switches[0],hosts[0],delay=str(delay)+"ms",bw=10) #
                delay = random.randint(1,1)
                self.addLink(switches[self.switch_num-1],hosts[1],delay=str(delay)+"ms",bw=10) #
            else:
                delay = random.randint(1,1)
                self.addLink(switches[count],switches[count+1],delay=str(delay)+"ms",bw=10) #,delay=str(delay)+"ms"
