...
class Network_Aware(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        '''
        msg = ev.msg
        the switch responds a feature reply message to feature request
        this msg has several attributes:
        datapath
        datapath_id
        n_buffers
        n_tables
        ...
        # self.logger.info("switch features received, switch:%s connected", datapath.id)

        # install table-miss flow entry
        # no rule to match then packet-in to the controller
   ...
   # get (src_dpid,dst_dpid)->(src_port,dst_port)
    def get_links(self):
    ...
    def create_port_map(self, switch_list):
        for sw in switch_list: # each switch
            dpid = sw.dp.id # switch's id
            self.switch_port_table.setdefault(dpid, set()) # dpid->port_num
            self.interior_ports.setdefault(dpid, set()) # dpid->port_num(interior ports)
            self.access_ports.setdefault(dpid, set()) #{(sw,port) :[host1_ip,host2_ip,host3_ip,host4_ip]}
            for p in sw.ports: # each port
                self.switch_port_table[dpid].add(p.port_no)
    ...
    def get_topology(self, ev):
        switch_list = get_switch(self.topology_api_app, None) # get all the switches
